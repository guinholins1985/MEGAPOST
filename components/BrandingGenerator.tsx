            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                {assets.map((asset, index) => (
                    <div key={`${assetType}-${index}`} className={`${aspectClass} rounded-lg border border-[var(--border-light)] dark:border-[var(--border-dark)] overflow-hidden bg-white shadow-sm transition-all duration-300 hover:shadow-xl hover:scale-105`}>
                        {asset === 'loading' ? <AssetLoader /> : asset ? (
...
const BrandingGenerator: React.FC<BrandingGeneratorProps> = ({ initialPrompt, sourceImage }) => {
    // Logo State
    ...
    useEffect(() => {
        if (sourceImage) {
            setReferenceImage({
                ...sourceImage,
                dataUrl: `data:${sourceImage.mimeType};base64,${sourceImage.base64}`
            });
        } else {
    ...
    const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onerror = () => {
          setBannersError("Falha ao ler o arquivo de imagem de referência.");
        }
    };
    
    const handleGenerateLogos = async () => {
        if (!logoPrompt.trim()) {
            setLogosError("Por favor, forneça uma descrição para a sua marca.");
            return;
        }
    ...
    const handleGenerateBanners = async () => {
        if (!bannerPrompt.trim()) {
            setBannersError("Por favor, forneça uma descrição para a sua marca.");
            return;
        }
    ...
    return (
        <div className="mt-12">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-2">Passo 2: Crie sua Identidade Visual</h2>
            <p className="text-center text-lg text-gray-600 dark:text-gray-400 mb-8">
                Descreva sua marca e gere logos e banners profissionais em segundos.
            </p>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                ...
                <div className="bg-[var(--card-light)] dark:bg-[var(--card-dark)] shadow-xl rounded-2xl border border-[var(--border-light)] dark:border-[var(--border-dark)] p-6">
                    ...
                    <textarea
                        ...
                        className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-900/50 border-2 border-[var(--border-light)] dark:border-[var(--border-dark)] rounded-lg focus:ring-2 focus:ring-[var(--primary-accent)] focus:border-transparent outline-none transition-all duration-300 resize-none"
                        rows={3}
