...
const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div className="bg-[var(--card-light)] dark:bg-[var(--card-dark)] p-6 rounded-2xl border border-[var(--border-light)] dark:border-[var(--border-dark)] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
    <div className="gradient-bg text-white rounded-lg h-12 w-12 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">{icon}</div>
        ...
        {!workspaceActive ? (
          <>
            ...
            <header className="text-center py-16 md:py-24 relative overflow-hidden">
                <div 
                  className="absolute inset-0 -z-10 [mask-image:radial-gradient(ellipse_at_center,white,transparent_60%)]"
                  style={{backgroundImage: 'radial-gradient(circle at 50% 50%, var(--primary-accent) 0%, transparent 30%), radial-gradient(circle at 10% 20%, var(--secondary-accent) 0%, transparent 25%), radial-gradient(circle at 90% 80%, var(--primary-accent) 0%, transparent 25%)', opacity: 0.15}}
                ></div>
                <h1 className="text-5xl md:text-7xl font-extrabold gradient-text leading-tight tracking-tighter">
                    MEGAPOST
                </h1>
                ...
                <button onClick={() => setWorkspaceActive(true)} className="mt-8 inline-block px-8 py-4 gradient-bg text-white font-bold rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--primary-accent)] transition-all duration-300 shadow-lg transform hover:scale-105 text-lg glow-on-hover">
          ...
          <div id="workspace" className="max-w-7xl mx-auto mt-8">
            <header className="flex items-center justify-between mb-8">
              <h1 className="text-2xl font-bold gradient-text">MEGAPOST</h1>
            ...
            <div className="space-y-12">
              <section>
                <h2 className="text-3xl md:text-4xl font-bold text-center mb-2">Passo 1: Forneça seu Produto</h2>
                <p className="text-lg text-gray-600 dark:text-gray-400 text-center mb-8 max-w-4xl mx-auto">Importe uma imagem, use a câmera ou cole um link para a IA analisar.</p>
